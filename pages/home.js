...
export default function Index() {
  ...
  async function setIPFSHash(newFileCID){
    ...
    const identifier = await contract.setStorageIdentifier(newFileCID);
    return identifier;
  }
  ...
  const fetchData = async () => {
    const response = await axios.post(
      ...
      {
        userAddress: localStorage.getItem("ACCOUNT_ADDRESS"),
  ...
  const handleCreateExpense = async() => {
    if (expenseName.length !== 0 && totalAmount !== 0 && Object.keys(paidBy).length !== 0 && Object.keys(splitBetween).length !== 0) {
      ...
      await storeFileCID();
      await axios.post('/api/addExpenseHandler', {
        paidBy: paidBy,
  ...
  const handleSettle = async ( to, amount, groupId) => {
    await storeFileCID();
    await axios.post('/api/settleHandler', {
                          ...
                          <List w='100%' >
                            {
                              ...
                              <Button onClick={
                               () => null} colorScheme={'teal'} >Settle</Button> 
